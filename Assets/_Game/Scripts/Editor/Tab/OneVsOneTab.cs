...
public class OneVsOneTab : TabContent
{
    ...
    {
        if (config == null || config.OneVsOneMode == null)
        {
            EditorGUILayout.HelpBox("OneVsOneMode not found!", MessageType.Error);
            return;
        }

        var mode = config.OneVsOneMode;
    ...
    private void DrawAllyFoldout(GameModeData mode, int index)
    {
        ...
        Draw.LabelBold("Ally Extra Data");
    ...
    private void DrawLevelGameFoldout(LevelGameData level, int index)
    {
        ...
        DrawEnemyFoldout(level, index); // ✅ Enemy nằm trong LevelGameData
